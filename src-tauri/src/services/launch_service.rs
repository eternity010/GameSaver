...
use crate::{
    app_state::AppState,
    domain::{
        Game, GameLifecycle, GameRuntime, GameRuntimeStatus, SaveProfile, SaveVersion, TaskStatus,
    },
...
impl LaunchService {
    ...
    pub fn launch(app: &AppHandle, state: &AppState, game_uid: String) -> Result<String, String> {
        ...
        let task_id =
            match TaskService::create(state, "launch_game", Some(game_uid.clone()), "准备启动游戏")
            {
                Ok(task_id) => task_id,
                Err(error) => {
                    if let Ok(mut running) = state.running_games.lock() {
                ...
                Ok(summary) => {
                        ...
                        .unwrap_or(false)
                    {
                        if let Some(version_id) =
                            summary.1.get("versionId").and_then(|v| v.as_str())
                        {
