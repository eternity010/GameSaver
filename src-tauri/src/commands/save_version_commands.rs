...
pub fn restore_save_version(
    ...
    std::thread::spawn(move || {
        let result = restore_version_task(
            &app_handle,
            &task_id_for_thread,
            &game,
            &profile,
...
pub fn delete_save_version(
    ...
    let task_id_for_thread = task_id.clone();
    std::thread::spawn(move || {
        let result = delete_versions_task(
            &app_handle,
            &task_id_for_thread,
            &game_uid,
        ...
        match result {
            Ok(summary) => {
                let message = if summary.get("garbageCollectionError").is_some() {
                    "保存版本已删除，但对象回收未完成"
                } else {
                    "保存版本已删除"
                };
...
pub fn prune_save_versions(
    ...
    std::thread::spawn(move || {
        let result =
            delete_versions_task(&app_handle, &task_id_for_thread, &game_uid, &version_ids);
        let state = app_handle.state::<AppState>();
        release_maintenance(&state, &game_uid);
        match result {
            Ok(summary) => {
...
> {
    ...
    let version = store
        .save_versions
        .iter()
        .find(|version| version.version_id == version_id && version.game_uid == game_uid)
        .cloned()
        .ok_or_else(|| "保存版本不存在".to_string())?;
    let latest = game
...
fn ensure_version_exists(state: &AppState, game_uid: &str, version_id: &str) -> Result<(), String> {
    ...
    if !store
        .save_versions
        .iter()
        .any(|version| version.game_uid == game_uid && version.version_id == version_id)
    {
        return Err("保存版本不存在".to_string());
    }
    Ok(())
