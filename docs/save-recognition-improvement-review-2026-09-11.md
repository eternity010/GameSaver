...
> **P3 / P4 已于 2026-09-13 实测结案（均不做）**：按本文档「等实测再做」的要求，用固化的测量夹具跑出真实数字后，P3 的「重复遍历」诊断被证伪（遍历 17ms vs 收集 2497ms），P4 的并行化收益被缓存快路径抵消 —— 理由与数字见 P3/P4 两条；实测顺带发现真正的瓶颈 **P5（逐文件 `canonicalize`，未实施）**。
> 仍未动的部分：**A1 的三项**、**P5**（P3 实测新发现）—— 见第 5 节的批次表。
...
- 证据：`add_candidate`（`save_repository.rs:1435-1439`）在**任何过滤之前**对每个走进来的文件调 `path.canonicalize()`；而它的调用点 `collect_profile_files`（`:1356-1367`）是**全目录、无深度上限**遍历。于是「目录里有多少文件」=「多少次 `canonicalize`」，与「其中几个是存档」无关 —— 被 `is_excluded` / `is_save_candidate` 拒掉的文件**照样**付了这次开句柄的代价。
- 量级（实测）：9660 文件 → 2497ms，其中约 2.4s 是 9660 次 `canonicalize`（Windows 上是 `GetFinalPathNameByHandle`，约 258µs/次）；同规模的 `collect_snapshot`（只 stat、不 canonicalize）只要 **415ms**（7200 个 / ≈58µs 个）。
- 修法（**未实施**，需单独一轮）：`root` 在 `collect_profile_files` 开头已经 canonicalize（`:1331-1335`），遍历出来的路径是 `root.join(...)`，相对路径可以**词法**算出 —— 已确认 `strip_prefix` 对 `root\.\bin\x` 这类路径仍能正确剥离，`normalize_relative`（`:1602-1612`）本来就会词法吃掉 `./`。所以最小改动是**把 `is_excluded` / `scope_admits_directory_file` 两道便宜的门提到 `canonicalize` 之前**，只对幸存者 canonicalize（本夹具 9660 → 2400，约 4×）；更彻底的是整段去掉逐文件 canonicalize，只存词法路径。
- **为什么单列、不顺手做**：`canonicalize` 现在承担的是 symlink/junction 与 8.3 短名的解析（`.` / `..` 已由 `normalize_relative` 词法处理），去掉它会改变 `relative_path` 的取值语义。本轮 R2b 已经证明「收集范围算错」的代价很高（会收进别人的存档），这类改动要配变异 + 软链/跨设备用例单独验证，不能夹在 P3 结案里顺手做。
- 状态：**未实施**（候选）。
...
| 备选 | **P3 / P4** 重复目录遍历、哈希阶段串行 | 纯性能，等实测 | 小 | **均已结案 2026-09-13**：P3「诊断不成立」、P4「收益不抵成本」，均**不做**；实测另发现真瓶颈 **P5 逐文件 `canonicalize`（未实施）**，见第 5 节 P3/P4 实测记录 |
