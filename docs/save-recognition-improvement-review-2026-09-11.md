...
> **A1 已于 2026-09-13 部分实施**：做了第 4 项「允许跳过学习」（只读初稿 + 前端旁路）与第 2 项「目录推断前置自动展示」；profile 形状复用与保存后自动分析**未做** —— 取舍理由见 A1 条。
...
> **P6 已于 2026-09-13 实施**（A1 第 2 项实测新发现）：`discover_scan_roots` 端到端 **194.5ms → 134.5ms（1.45×）**；这条也是「隔离基准会骗人」的实例 —— 第一版真实现反而慢 15%，经过见 P6 条。
> 仍未动的部分：**A1 的两项**（profile 形状复用、保存后自动分析）与 **`is_save_candidate` 的开销** —— 见第 5 节的批次表。
> ⛔ **2026-09-13 主人决定：这三项「成本高收益小的暂时先不做」**，全部挂起。理由分别见 A1 条（前两项）与 P5 条（`is_save_candidate`）；恢复条件是主人明说要动，或出现新的实测证据表明收益变大。**本文档此前的"仍未动"不等于"待办"。**
...
- **已实施（2026-09-13）**：只做了第 4 项，第 1/3 项未做，第 2 项算部分落地。做法与取舍：
  ...
  - **第 2 项（目录推断前置）已于 2026-09-13 补做「自动展示」**：进入 `ready`（本体复制完成）后自动跑一次只读推断，在 `ready` 阶段直接列出候选目录（根类型 + 路径），并给「查看并编辑这份初稿」入口 —— 复用已取回的结果，不再二次请求（避免「点下去又等一次遍历、还可能拿到与刚才展示不一致的清单」）。
    - **实现为「后台预取」**：用独立的 `inferringDrafts`，**不并入 `previewing`** —— 推断期间「启动并开始识别」必须仍可点，否则自动展示就把主流程堵住了。同一 `gameUid` 只自动推断一次（`inferredForGameUid` 守卫）：`phase` 在学习失败/取消后会退回 `ready`，不该重新遍历一遍目录。
    - **失败静默**：只读推断是锦上添花，失败就少一块提示（面板不出现），不写 `error`、不影响继续走完整识别。
    - **证据仍诚实**：面板不显示置信度评分（只读推断没有写入证据），只给目录清单 + 「这是**只读推断**：没有启动游戏、没有任何写入证据」；空状态沿用 A3 的口径说清病因（「候选目录名里没有出现游戏名时，普通权限发现不了它」）。
    - ⚠️ **代价实测（8 个真实游戏，本机 store）**：原先按合成夹具估的「约 600–700ms」**是错的**，真实代价高得多，而且**几乎全在 `discover_scan_roots`，不在快照**：

      | 游戏 | 根 | 文件 | discover | snapshot | 合计 |
      | --- | --- | --- | --- | --- | --- |
      | DemonicMahjong | 2 | 57 | **1870.9ms** | 4.6ms | **1875.5ms** |
      | 心跳！性感按摩 | 2 | 24 | 589.0ms | 15.1ms | 604.1ms |
      | 傻乎乎主播蔚来 | 1 | 4870 | 325.0ms | 221.9ms | 546.9ms |
      | 推币机☆Pussy | 3 | 173 | 411.1ms | 17.0ms | 428.2ms |
      | 强行枕营业！2 | 2 | 181 | 369.3ms | 21.1ms | 390.4ms |
      | 补丁游戏 | 1 | 1334 | 205.2ms | 61.5ms | 266.7ms |
      | Prison v1.0.1 | 1 | 215 | 202.6ms | 6.7ms | 209.3ms |
      | 人妻LovelyMoment | 1 | 22 | 114.8ms | 1.3ms | 116.1ms |

      区间 **116ms – 1.9s**（中位数约 410ms）。注意「文件少但根多」的 DemonicMahjong 反而最慢 —— 说明开销在**逐根的目录探测/匹配**，与文件数无关。这与 P3 的教训同型（第二次应验）：**别按结构估算，实测才知道**。
      ⚠️ 上表是**冷缓存**下的读数（同一游戏热缓存只要约 200ms，差近 10 倍）。后续 P6 的端到端测量都在可比条件下做。
    - 当初「不自动展示」的理由（用户没要求时不该先付）**因为这份实测而更有分量**。本次**按主人明确要求**改成自动，取舍已从「省一次扫描」改为「早一步知道目录对不对」；随后的 **P6 已把这个瓶颈压掉约 1/3**（194.5ms → 134.5ms），所以现在这笔代价比上表小得多 —— **优化发现环节，而不是删掉前置推断**，这条路走通了。
    - **未加单元测试**：纯前端行为，而本仓库没有前端测试框架（既有前端验证方式就是 `vue-tsc` + `vite build` 门禁）。诚实记录：面板的交互路径没有自动化断言。
...
### P6. `discover_scan_roots` 的逐 hint 重复判断（中，A1 第 2 项实测新发现）

- 证据：`find_candidate_directories`（改动前）把「噪声目录判断 / 存档容器目录判断 / 目录名 `to_lowercase` / 去非字母数字 / `is_generic_hint` 黑名单查询」全塞在 `hints.iter().any(|hint| directory_matches_hint(&path, hint))` 的**逐 hint 调用里** —— 而这几件事与 hint **毫无关系**。于是同一份工作被重复了「提示词个数」遍（真实游戏 4~5 个提示词）。
- 暴露方式：A1 第 2 项「目录推断前置自动展示」把只读推断从按钮改成自动后，实测每次添加游戏要代价；量到 **`discover_scan_roots` 吃掉了几乎全部**（`collect_snapshot` 只要 4.6ms）。
- **已实施（2026-09-13）**：新增 `DirectoryHintMatcher`，把与 hint 无关的检查与规范化**每个目录只做一次**；通用词提示词在 `find_candidate_directories` 入口**每根剔一次**并共享切片。
  - **端到端实测（真实 store 的 4 个真实游戏 × 4 次均值，worktree 内旧/新各测）**：**194.5ms → 134.5ms（1.45×）**；逐游戏 DemonicMahjong 231.6→147.3、Prison 197.2→125.8、补丁游戏 192.6→143.5、人妻LovelyMoment 156.5→121.5。候选目录集合逐个路径比对**完全一致**。
  - ⚠️ **过程中我自己踩了两个坑，都记在这里**（因为都差点被当成结论留下）：
    1. **隔离基准会骗人**：我先写了一份「把匹配逻辑复制出来」的基准，测出 **1.9×**；但把它当真实现跑端到端时，第一版**反而从 185.7ms 涨到 213.0ms（慢 15%）**。原因是我在匹配器构造函数里 `cloned()` 收了一份 `Vec<String>` 提示词 —— **每个被检视的目录都克隆一遍全部提示词**（真实机器约 1 万多个目录）。隔离基准把这份克隆提到了循环外，所以完全没看见。**教训：复制出来的基准测的是你写的那份副本，不是真函数。**
    2. **缓存状态会淹没结论**：同一份代码在冷/热文件系统缓存下读数差近 10 倍（同一游戏 1.87s vs 0.20s），早期有一批数字因此不可比。**引用这类数字必须说明是怎么测的**（见 principles §7.6）。
- 状态：**已实施**。`find_steam_userdata_dirs` 的 52 个盘符探测实测仅 0.4ms（不存在的盘瞬时失败），**不是**瓶颈，无需动。
...
| 第 5 批 | **A1** profile 形状复用、目录推断前置、保存后自动分析 | 自动化，涉及前端 | 大 | **部分实施 2026-09-13**（已做第 4 项「跳过学习」+ 第 2 项「目录推断前置自动展示」；profile 复用 / 自动分析**未做**，见 A1 条） |
...
| 第 7 批 | **P6** `discover_scan_roots` 的逐 hint 重复判断 | 纯性能，行为等价 | 小 | **已实施 2026-09-13**：端到端 194.5ms → 134.5ms（**1.45×**），2 条守卫测试；A1 第 2 项实测发现，见第 5 节 P6 落地记录 |
...
**第 7 批（P6）落地记录（2026-09-13）**：A1 第 2 项落地后实测其代价，发现几乎全在 `discover_scan_roots`，于是做了这一批。改动：新增 `DirectoryHintMatcher`，把「噪声目录 / 存档容器目录 / 目录名规范化 / `is_generic_hint`」这些**与 hint 无关**的检查从逐 hint 调用里提出来，每个目录只做一次；通用词提示词在 `find_candidate_directories` 入口每根剔除一次并共享切片。

实测（真实 store 的 4 个真实游戏 × 4 次均值；旧/新代码各在 `HEAD~1` 与 `HEAD` 的 worktree 里跑同一份测量）：

| 游戏 | 旧 | 新 | 加速 |
| --- | --- | --- | --- |
| DemonicMahjong | 231.6ms | 147.3ms | 1.57× |
| Prison v1.0.1 | 197.2ms | 125.8ms | 1.57× |
| おかしな刑務所…简体中文补丁 | 192.6ms | 143.5ms | 1.34× |
| 人妻LovelyMoment | 156.5ms | 121.5ms | 1.29× |
| **平均** | **194.5ms** | **134.5ms** | **1.45×** |

候选目录集合逐个路径比对**完全一致**。守卫测试 2 条（`services::save_learning_service::tests`）：

| 测试 | 钉住的性质 |
| --- | --- |
| `find_candidate_directories_rejects_save_containers_noise_and_generic_names` | 存档容器 / 噪声目录 / 通用词目录名仍然被排除，命中的真实目录仍在 |
| `rejection_does_not_stop_descending_into_deeper_candidates` | 被排除的目录**仍要继续下探**（「不是候选」≠「不下探」，本批最容易写错的一处）|

> ⚠️ **本批最重要的一条不是代码，而是「隔离基准骗了我」**：我先写了一份把匹配逻辑复制出来的基准，测出 **1.9×**；但把同一改动接进真实函数跑端到端时，**第一版反而从 185.7ms 涨到 213.0ms（慢 15%）**。根因是我在匹配器构造函数里 `cloned()` 收了一份提示词 `Vec`，于是**每个被检视的目录都克隆一遍全部提示词**（真实机器约 1 万多个目录）；而隔离基准恰好把这份克隆提到了循环外，所以完全没看见。
> **两条可复用教训**：①**复制出来的基准测的是你写的那份副本，不是真函数** —— 改写热路径后必须回真实函数上做端到端 A/B；②被优化代码的**分配次数乘以调用次数**才是成本，一次多余分配在「每目录一次」的热路径上就是明显回退。

门禁：`cargo fmt --check` 干净、`cargo test --lib` **276 passed** / 0 failed / 2 ignored（275 → 276，本批净增 1 条：删掉 1 条直接调内部函数的旧测试、新增 2 条走真实入口的守卫）。
