...
| C4 | 低 | **待办（性质已变）** | 云同步命令层（`cloud_save_commands.rs`）**没有任何测试**；逐行读过后发现里面还有真缺陷（见 H3），所以它不再只是「覆盖缺口」 | 整个文件 414 行 |
...
| H2 | 中 | ✅ 已修（§0.6） | `rebuild` 用 `.ok().flatten()` 把「清单读失败」当成「清单不存在」，覆盖回写会静默丢掉 `sha256`/`file_count`/`total_bytes` → 该版本此后下载不再比对哈希 | `cloud_manifest_service.rs:469` |

> 云端存档侧只剩 C4（覆盖缺口）；云本体侧 H1/H2 已修，另有 §7 的 **L1 / L2 / L3** 与一处疑似死代码**尚未处置**。已修的六条里，C5、H3 都属「一旦发生就要用户承担后果」（永久卡住 + 文案误导），C1、C3、H1、H2 是加固与静默降级。值得注意的是：**H3 是 C2 自己带进来的回归**，性质与它要修的 C5 同类。处置顺序见 §6。
...
| 情况 | 处置 | 理由 |
|---|---|---|
| 取不下来 / 解析不了 | **不覆盖**，返回错误 | 旧清单可能完好，这一趟只是网络不顺；覆盖等于静默丢元数据 |
| 远端列表里确实没有清单 | 按列表新建 | 正常路径 |
| 能解析但校验不过 | **继续重建**，沿用其中元数据 | 这正是「修复清单」要修的场合 |

**做法**：把 `read` 拆出 `fetch`（只下载 + 解析，**不校验**），`read = fetch + validate + 缓存`，`rebuild` 直接用 `fetch` 并对上面三种情况分开处置。
...
- 门禁：`cargo fmt --check` 干净、`cargo test --lib` **306 passed / 0 failed / 2 ignored**、clippy 0 error 且警告数与基线持平（34）、`npm run build` 通过。
...
>
> 所以「零测试」在这里不是抽象的覆盖率问题，而是**它掩盖了一个与 C5 同级的后果**。不过要注意：H3 是**逐行读**发现的，不是测试发现的 —— 说明这一层的正确打开方式首先是读，其次才是补测试。补测试的价值集中在**任务编排**（进度推进、重试载荷、失败/取消的状态迁移），而这需要 `&AppHandle` 接缝（同 `8d09f9f`）。
...
| 第 7 批 | ~~**H3** `cloud_save_commands.rs` 的认领泄漏（C2 回归）改用 RAII 守卫；**H2** `rebuild` 区分「读不到清单」与「清单不存在」~~ **已完成（见 §0.6）** | 修自己的回归 + 小改 |
| 第 8 批 | **C4** 为 `cloud_save_commands.rs` 补编排层测试 | 补验证 |
...
**C4 的剩余价值已被前几批改变**：它原本的建议是「补编排层测试，可同时钉住 C2 的加锁行为」。C2 落地后**互斥语义已由 `app_state` 的测试钉住**（含双向排斥与持锁判定一致性），C3 的临时文件生命周期也有了测试，H3 的守卫释放时机现在也有 2 条测试。所以 C4 现在只剩**任务编排**（进度推进、重试载荷、失败/取消的状态迁移）这块未覆盖 —— 价值仍在，但已明显低于 H3 那类真缺陷。要测这一层需要 `&AppHandle` 接缝（同 `8d09f9f` 的做法）。
