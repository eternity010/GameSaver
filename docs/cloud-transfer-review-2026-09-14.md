...
> 云端存档侧 C1–C5 全部收口；云本体侧 H1/H2 已修，另有 §7 的 **L1 / L2 / L3** 与一处疑似死代码**判定为不值得做**。已修的七条里，C5、H3 都属「一旦发生就要用户承担后果」（永久卡住 + 文案误导），C1、C3、H1、H2 是加固与静默降级。**两点值得单独记住**：H3 是 C2 自己带进来的回归，性质与它要修的 C5 同类；而 H3 是**逐行读出来的、不是测试抓到的**（C4 那 3 条补测上线时 H3 已修）。处置顺序见 §6。**收口状态与「没做什么」的边界见 §8** —— 那里的结论是「本轮条目已全部处置」，不是「云模块已被证明安全」。
...
- **云本体包解压（zip-slip）**：**两侧都堵住了**。解压是**清单驱动**的 —— `extract_package_with_known_hash` 遍历 `manifest.files`、对每条路径调 `safe_join(staging_root, ..)`（`game_body_package_service.rs:714`），而**不**遍历 zip 条目；清单里的 `relative_path` 过 `normalize_relative`（`:1133-1153`，拒绝绝对路径 / `ParentDir` / `RootDir` / `Prefix` / NUL / 空串，fail-closed）。反向也被堵：`validate_archive_entries`（`:1198-1216`）会把**每个 zip 条目名**同样归一化，并要求它落在清单声明的集合里（多一个条目、重复条目都报错）。所以「改坏清单」与「塞额外条目」两条路都到不了 `staging_root` 之外。**这一条是 2026-09-14 为回答「能否收口」专门补查的，此前只在存档包那侧查过穿越。**
...
## 8. 收口状态与边界（2026-09-14）

**结论：这一轮审计找出的条目已全部处置完，没有留下已知的可达缺陷。**

- C1–C5、F1、H1–H3 **全部已修**，每条都带测试与变异验证（见各 §0.x 的「测试与变异」）；
- L1 / L2 / L3 与一处疑似死代码**判定为不值得做**，理由写在 §6 末段 —— 是「已知且决定不处理」，不是「忘了」；
- §5 存疑项里唯一保留的服务端未知（百度对 `..` 的归一化）**已与我们的风险无关**，因为我们不再把未证明属于本游戏的路径发出去。

### 但「收口」不等于「云模块已被证明安全」

必须写清边界，否则上面那句话会被读成后者：

| 没做的 | 说明 |
|---|---|
| **前端未审计** | 整个审计是 Rust 侧的。云同步 / 云本体的前端调用面（`App.vue`、`TransferCenter.vue`、`GameStorePage.vue`）只看了传什么参数，**没有审前端逻辑本身** |
| **依赖供应链未查** | `cargo audit` / `npm audit` 都不在本轮范围内 |
| **无端到端 / 真实网盘验证** | 全部结论是静态阅读 + 单元测试 + 变异。没有真实账号走过安装 / 上传 / 恢复 / 剪枝 / 修复清单 |
| **L1–L3 未处理** | 已知、判低价值、可达性未证实 |
| **服务端行为未知** | 百度对 `..` / `.` / 空路径段的归一化仍无文档或实测（已不影响我们） |

### 一条方法论：本轮出现了两次「自己引入的回归」

- **H3**：C2 修「并发丢条目」时把认领加进 `cloud_save_commands.rs`，漏了三处 `?` 早退的释放 → 云同步永久卡死（与它要修的 C5 同类后果）。
- **H2 第一版**：修「读失败当成清单不存在」时，把「解析不了」也归成「取不下来」，**堵死了修复工具**（§0.6 补正）。

两次都是靠**重读代码**发现的 —— 不是靠清单打勾，也不是靠测试。所以支撑「收口」的不是「条目都勾上了」，而是：每条修复都配了测试与变异，且这几轮确实把相关代码读透了。**反过来说，任何一次「按清单补覆盖率」式的动作都不足以支撑收口**（§0.7 里 C4 的教训同此）。

### 若要继续加强，性价比最高的三步

1. **真实网盘端到端走一遍关键路径**（安装云本体 / 上传 / 恢复 / 剪枝 / 修复清单）—— 这是唯一能覆盖「真实网络 + 真实服务端行为」的手段，也是本轮所有结论的共同盲区；
2. **前端调用面核对**传参（部分已在 Rust 侧兜住，但前端自身的状态机没看）；
3. **`cargo audit`** 一把。